...
class GameHTMLContent {
    ...
    private static readonly FILLED_CELL = `
    ...
    private static readonly GAME_TITLE = `
        <h1 id="gameTitle" class="text-slate-400 text-center text-4xl font-['Teachers']">%s</h1>
    `;
    
    public static getFilledCellHTML(player: PlayerType): string {
        return GameHTMLContent.FILLED_CELL.replace('%s', player.symbol);
    }
    
    public static getGameTitleHTML(title: GameTitle): string {
        return GameHTMLContent.GAME_TITLE.replace('%s', title);
    }
